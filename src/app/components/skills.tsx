...
function SkillItem({ category, items } : SkillItemProps) {
  ...
  return (
    <>
      <div className="font-semibold">{category}<span className="float-right max-sm:hidden">:</span></div>
      <div>{skills}</div>
    </>
  );
}
  ...
  return (
    <section className="flex min-h-0 flex-col gap-y-3 print:gap-y-1">
      ...
      <div className="mt-2 text-sm font-mono text-foreground/80 print:mt-1 print:text-[12px] text-pretty">
        <div className="grid grid-cols-[28%_72%] max-sm:grid-cols-1 gap-1 print:grid-cols-[25%_75%] print:gap-0.5">
